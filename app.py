...
def extract_data_from_pdf(pdf_file):
    """
    Fungsi untuk mengekstrak data dari file PDF dan memastikan urutan data sesuai dengan tabel di dalamnya.
    """
    data = []
        ...
        for page in pdf.pages:
            ...
            if text:
                try:
                    ...
                    # Menangkap semua baris barang berdasarkan nomor urut di tabel
                    barang_rows = re.findall(r'(?m)^\s*(\d+)\s+(\d+)\s+(.+?)\s+Rp ([\d.,]+) x ([\d.,]+) Bulan', text)
                    
                    for row in barang_rows:
                        no_urut, kode_barang, nama_barang, harga, qty = row
                        harga = int(float(harga.replace('.', '').replace(',', '.')))
                        qty = int(float(qty.replace('.', '').replace(',', '.')))
                        total = harga * qty
                        unit = "Bulan"

                        # Menangkap nilai pajak
                        ...
                        data.append([no_urut, kode_barang, nama_barang, harga, unit, qty, total, dpp, ppn, tanggal_faktur])
...
if uploaded_files:
    ...
    if all_data:
        df = pd.DataFrame(all_data, columns=["No Urut", "Kode Barang", "Barang", "Harga", "Unit", "QTY", "Total", "DPP", "PPN", "Tanggal Faktur"])
