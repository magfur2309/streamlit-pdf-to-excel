        ...
        for page in pdf.pages:
            ...
            if text:
                try:
                    ...
                    # Menangkap informasi barang/jasa sesuai format yang benar
                    barang_pattern = re.findall(r'(?P<kode>\d+)\s+(?P<barang>[A-Za-z0-9\-\s\/]+)\s+Rp\s+(?P<harga>[\d.,]+)\s+x\s+(?P<qty>[\d.,]+)\s+(?P<unit>\w+)', text)
                    if barang_pattern:
                        for barang_match in barang_pattern:
                            kode, barang, harga, qty, unit = barang_match
                            ...
                            page_data.append([faktur_counter, no_fp, nama_penjual, nama_pembeli, kode, barang.strip(), harga, unit, qty, total, dpp, ppn, tanggal_faktur if tanggal_faktur else "Tidak ditemukan"])
                    else:
                        page_data.append([faktur_counter, no_fp, nama_penjual, nama_pembeli, "Tidak ditemukan", "Tidak ditemukan", 0, "", 0, 0, 0, 0, tanggal_faktur if tanggal_faktur else "Tidak ditemukan"])
...
if uploaded_files:
    ...
    if all_data:
        df = pd.DataFrame(all_data, columns=["No", "No FP", "Nama Penjual", "Nama Pembeli", "Kode Barang", "Barang", "Harga", "Unit", "QTY", "Total", "DPP", "PPN", "Tanggal Faktur"])
