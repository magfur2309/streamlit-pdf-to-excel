        ...
        for page in pdf.pages:
            ...
            if text:
                try:
                    ...
                    # Ekstraksi Barang/Jasa berdasarkan tabel
                    barang_matches = re.findall(r'(?m)^(\d+)\s+(\d+)\s+([\w\s]+)\s+Rp ([\d.,]+) x ([\d.,]+) Bulan', text)
                    ...
                    for match in barang_matches:
                        nomor, kode, barang, harga, qty = match
                        ...
                        data.append([no_fp, nama_penjual, nama_pembeli, nomor, kode, barang, harga, unit, qty, total])
...
if uploaded_files:
    ...
    if all_data:
        df = pd.DataFrame(all_data, columns=["No FP", "Nama Penjual", "Nama Pembeli", "No", "Kode", "Barang", "Harga", "Unit", "QTY", "Total"])
