...
def get_device_id():
    """Fungsi sederhana untuk mengidentifikasi perangkat pengguna."""
    return st.session_state.get("device_id", str(hash(st.session_state.get("username", ""))))

if "demo_upload_count" not in st.session_state:
    st.session_state["demo_upload_count"] = {}
...
def submit_auth():
    ...
    if username == demo_user and password == demo_password:
        if st.session_state["demo_upload_count"].get(device_id, 0) >= demo_upload_limit:
...
def extract_data_from_pdf(pdf_file, existing_faktur, uploaded_files):
    """
    ...
    with pdfplumber.open(pdf_file) as pdf:
        for page in pdf.pages:
            ...
            if text:
                ...
                if no_fp:
                    if no_fp in existing_faktur and existing_faktur[no_fp] != tanggal_faktur:
                        st.error(f"Nomor faktur {no_fp} memiliki tanggal berbeda sebelumnya: {existing_faktur[no_fp]}")
                        continue
                    if no_fp in uploaded_files:
                        st.error(f"Nomor faktur {no_fp} sudah ada di file PDF lain!")
                        continue
                    
                    existing_faktur[no_fp] = tanggal_faktur
                    uploaded_files.add(no_fp)
...
if authenticate():
    ...
    if uploaded_files:
        ...
        if st.session_state["username"] == demo_user:
            if st.session_state["demo_upload_count"].get(device_id, 0) + len(uploaded_files) > demo_upload_limit:
                st.error("Batas unggahan akun demo tercapai. Minta akses tambahan.")
            else:
                st.session_state["demo_upload_count"][device_id] = st.session_state["demo_upload_count"].get(device_id, 0) + len(uploaded_files)
        
        all_data = []
        existing_faktur = {}
        uploaded_faktur_set = set()
        
        for uploaded_file in uploaded_files:
            extracted_data = extract_data_from_pdf(uploaded_file, existing_faktur, uploaded_faktur_set)
            if extracted_data:
